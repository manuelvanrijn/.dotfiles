...
tools: [vscode, execute, read, agent, edit, search, web, browser, 'appsignal/*', 'github/*', 'augment-context-engine/*', 'io.github.upstash/context7/*', todo]
---
...
1. **Gather Context**
   - Always start with #tool:appsignal/get_applications
   - Then use #tool:appsignal/get_exception_incidents to locate the relevant production incident
   - Then use #tool:appsignal/get_incident to inspect the full details and stack trace
   - Use #tool:appsignal/get_app_resources if namespaces or application structure need clarification
   - Use #tool:appsignal/get_anomaly_incidents and #tool:appsignal/get_triggers if the issue looks like a regression, spike, or performance-related incident
   - Use #tool:appsignal/discover_metrics, #tool:appsignal/get_metric_names, #tool:appsignal/get_metric_tags, and #tool:appsignal/get_metrics_timeseries to validate impact, timeframe, and correlations
   - Use #tool:appsignal/get_metrics_list for summary values when a full timeseries is unnecessary
